...
include ("../../../config/conexion_bd.php");
    ...
    $query = "SELECT Nombre_intercambio, Descripcion_intercambio, Imagen_intercambio, Categoria_intercambio, Articulo, Precio_intercambio, Celular, Nombre_ca, Nombre FROM intercambio INNER JOIN usuarios ON Id_usuario = Usuario_ofertante INNER JOIN categoria ON Id_categoria = Categoria_intercambio INNER JOIN Articulo ON Id_articulo = Articulo WHERE Id_intercambio = '$product_id'";
...
<body>
    ...
    <?php while ($row = $resultado->fetch_assoc()) { ?>
        <div class="container-producto">
            <div>
            <img src="<?php echo '../../../public/assets/images/images_productos/' . $row['Imagen_intercambio']; ?>" alt="Producto" class="imagen-product-select">
            </div> 
                <div class="info-product-select">
                <div class="cat-product-select">
                    <h2 class="name-product-select">Nombre oferta: <?php echo $row['Nombre_intercambio']; ?></h2>
                    <h2 class="categoria"> <?php echo $row['Nombre_ca']; ?></h2>
                </div>
                <div class="valor-swap">
                    <h3 class="precio">Articulo interesado: <?php echo $row['Nombre']; ?> </h3>
                    <h3 class="precio">Precio ofertado: <?php echo $row['Precio_intercambio']; ?> </h3>
                    <h3 class="swap">Descripcion del producto: <?php echo $row['Descripcion_intercambio']; ?> </h3>
                </div>
                
                <h3>Si te interesa la oferta llama o escribe al siguiente numero <?php echo $row['Celular']; ?><h3>
                </div>
            </div>
        </div>
        <?php } ?>
